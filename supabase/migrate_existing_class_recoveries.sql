...
alter table public.class_recoveries
  add column if not exists recovery_date date;

alter table public.class_recoveries
  add column if not exists attendee_type text default 'self';
...
alter table public.class_recoveries
  add column if not exists attendee_dependent_id bigint;
...
update public.class_recoveries
set attendee_type = 'self'
...
update public.class_recoveries
set recovery_date = current_date + 2
...
begin
  if exists (
    select 1
    from pg_constraint
    where conname = 'class_recoveries_status_check'
      and conrelid = 'public.class_recoveries'::regclass
  ) then
    alter table public.class_recoveries
      drop constraint class_recoveries_status_check;
  end if;
end $$;

alter table public.class_recoveries
  add constraint class_recoveries_status_check
...
begin
  if exists (
    select 1
    from pg_constraint
    where conname = 'class_recoveries_attendee_type_check'
      and conrelid = 'public.class_recoveries'::regclass
  ) then
    alter table public.class_recoveries
      drop constraint class_recoveries_attendee_type_check;
  end if;
end $$;

alter table public.class_recoveries
  add constraint class_recoveries_attendee_type_check
  check (attendee_type in ('self', 'dependent'));

drop index if exists class_recoveries_student_date_class_unique;
